...
if "t12_data"         not in st.session_state: st.session_state["t12_data"]          = None
if "rr_data"          not in st.session_state: st.session_state["rr_data"]           = None
if "t12_property_id"  not in st.session_state: st.session_state["t12_property_id"]   = None
if "rr_property_id"   not in st.session_state: st.session_state["rr_property_id"]    = None
if "view_mode"        not in st.session_state: st.session_state["view_mode"]         = "Analyst/Admin"
...
with st.sidebar:
    ...
    # ── Client selector ───────────────────────────────────────────────────
    st.markdown('<p style="color:#8BA3C7;font-size:11px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;">CLIENT</p>', unsafe_allow_html=True)
    ...
    # ── Property selector ─────────────────────────────────────────────────
    st.markdown('<p style="color:#8BA3C7;font-size:11px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;">PROPERTY</p>', unsafe_allow_html=True)
    ...
    with st.expander("+ New Property", expanded=not properties):
        ...
        new_prop_type  = st.selectbox("Property Type", ["Multifamily","Mixed Use","Student Housing","Senior Housing","BTR"], key="new_prop_type")
    ...
    if property_id:
        ...
        if prop:
            st.markdown(f"""
            <div style="background:#0D1526;border:1px solid #1E2D4A;border-radius:8px;padding:10px;margin-top:8px;">
                <p style="color:#F0F4FF;font-size:12px;font-weight:600;margin:0;">{prop['name']}</p>
                <p style="color:#8BA3C7;font-size:11px;margin:3px 0 0 0;">{prop.get('city','')}, {prop.get('state','')}</p>
                <p style="color:#8BA3C7;font-size:11px;margin:2px 0 0 0;">{prop.get('total_units',0)} units | Class {prop.get('asset_class','—')}</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    # ── Navigation ────────────────────────────────────────────────────────
    st.markdown('<p style="color:#8BA3C7;font-size:11px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;">NAVIGATION</p>', unsafe_allow_html=True)
    ...
    for icon, page in nav_pages:
        ...
        style = "background:#1E6FEB;border-radius:6px;" if active else ""
        if st.button(
            f"{icon}  {page}",
            key=f"nav_{page}",
...
# Filter data to current property
...
# ── Page: Overview ────────────────────────────────────────────────────────────
if active == "Overview":
    _render_page_header("Portfolio Overview", "🏢")
...
# HELPER FUNCTIONS
# ═══════════════════════════════════════════════════════════════════════════════
def _render_page_header(title: str, icon: str = ""):
...
def _render_insights_panel(t12_data, rr_data, property_id):
    ...
    if insights:
        ...
        for ins in insights:
            ...
            metric = f"**{ins.get('metric_label')}:** {ins.get('metric_value')}" if ins.get("metric_label") else ""
            st.markdown(f"""
...
def _trigger_export(client_id, property_id):
    ...
    fname = f"CRE_Dashboard_{prop['name'] if prop else 'Export'}_{datetime.now().strftime('%Y%m%d')}.xlsx"
...
def _render_settings():
    ...
    **To add Supabase Auth:**
    ...
    3. Replace `database/db.py` connection factory with Supabase client
    4. Wrap all pages with `st.session_state['user']` checks
    5. Implement Row Level Security on Supabase tables using `client_id` and `property_id` foreign keys
    """)
